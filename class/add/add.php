<?php
	...
	class add {
			...
			public function add_equipment()
		{
            ...
            $e_grams = $_POST['e_grams'];
            $e_liter = $_POST['e_liter'];

			session_start();
			$h_desc = 'add new Item'. $e_model.' , '.$e_category;
			$h_tbl = 'equipment';
			$sessionid = $_SESSION['admin_id'];
			$sessiontype = $_SESSION['admin_type'];
           ...
           $sql = $conn->prepare('INSERT INTO item (i_deviceID, i_model, i_category, i_brand, i_description, i_type, item_rawstock, i_mr, i_price, i_date_added, i_date_expiration, i_grams, i_liter)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
           $sql->execute(array($e_number, $e_model, $e_category, $e_brand, $e_description, $e_type, $e_stock, $e_mr, $e_price, $e_date_added, $e_date_expiration, $e_grams, $e_liter));
           $row = $sql->rowCount();
		...
		public function add_member($as, $handle)
{
    ...
    try {
        ...
        // Convert m_type to "student" or "faculty"
        $memberType = ($as == 1) ? 'student' : 'faculty';

        $insert = $conn->prepare('INSERT INTO history_logs(description, table_name, user_id, user_type) VALUES(?,?,?,?)');
        $insert->execute(array($h_desc, $h_tbl, $sessionid, $memberType));
...
public function add_borrower($name, $item, $id, $reserve_room, $timeLimit)
{
    ...
    if ($row == 3) {
        echo json_encode(array("response" => 0, "message" => 'Unable to process your transaction. Please return first your borrowed items'));
    } else {
        $borrowIds = array();
        ...
        foreach ($item as $key => $items) {
            ...
            // Add this line at the beginning of your PHP code to get the selected quantity
            $quantity = isset($_POST['quantity']) ? intval($_POST['quantity']) : 1;

            if ($quantity <= 0) {
                echo json_encode(array("response" => 0, "message" => 'Invalid quantity.'));
                exit();
            }

            // Check if the requested quantity is higher than the available stock
            $stockCheck = $conn->prepare('SELECT items_stock FROM item_stock WHERE id = ?');
            $stockCheck->execute(array($itemsArr[1]));
            $currentStock = $stockCheck->fetchColumn();

            if ($quantity > $currentStock) {
                echo json_encode(array("response" => 0, "message" => 'Not enough quantity in stock.'));
                exit();
            }

            $sql = $conn->prepare('INSERT INTO borrow (borrowcode, member_id, item_id, stock_id, user_id, room_assigned, time_limit, quantity) VALUES(?,?,?,?,?,?,?,?)');
            $sql->execute(array($code, $name, $itemsArr[0], $itemsArr[1], $id, $reserve_room, $timeLimit, $quantity));
            ...
            if ($count > 0) {
                ...
                $update->execute(array($quantity, $itemsArr[1]));

                if ($update->errorCode() !== "00000") {
                    $errorInfo = $update->errorInfo();
                    echo json_encode(array("response" => 0, "message" => 'Error updating item stock: ' . $errorInfo[2]));
                    exit();
                }
            }
        }
        ...
        if ($history->errorCode() !== "00000") {
            $errorInfo = $history->errorInfo();
            echo json_encode(array("response" => 0, "message" => 'Error logging history: ' . $errorInfo[2]));
            exit();
        }
...
public function addclient_reservation($items, $date, $time, $client_id, $assign_room, $timeLimit)
{
    ...
    if ($client_id == 0) {
        ...
        try {
            foreach ($items as $key => $item) {
                ...
                $sql1 = $conn->prepare('INSERT INTO reservation(reservation_code, member_id, item_id, stock_id, reserve_date, reservation_time, assign_room, time_limit, quantity) VALUES(?,?,?,?,?,?,?,?,?)');
                $sql1->execute(array($code, $client_id, $itemsArr[0], $itemsArr[1], $date, $time, $assign_room, $timeLimit, $quantity));
                $count += $sql1->rowCount(); // Add to the total count
	...
	switch ($key){
		...
		$sid_number = trim($_POST['sid_number']);
 	    ...
 	    $s_password = isset($_POST['s_password']) ? trim($_POST['s_password']) : null;
 		$type = 'Student';
		...
		$f_department = trim($_POST['f_department']);
		 $f_password = isset($_POST['f_password']) ? trim($_POST['f_password']) : null;
 		$type = 'Faculty';
