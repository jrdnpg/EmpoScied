...
public function return_item($id)
{
    ...
    foreach ($fetch as $key => $value) {
        ...
        $quantityBorrowed = $value['quantity']; // Fetch the quantity borrowed
        ...
        $sql->execute(array(2, $date, $borrowcode, $quantityBorrowed, $equip));
        $count = $sql->rowCount();
        $datakey = $key + 1;
	...
	public function borrowreserve($code)
{
    ...
    if ($num > 0) {
        ...
        foreach ($fetch as $key => $value) {
            ...
            $checkBorrowed = $conn->prepare('SELECT COUNT(*) FROM borrow WHERE item_id = ? AND status = 1');
            $checkBorrowed->execute(array($value['item_id']));
            $alreadyBorrowed = $checkBorrowed->fetchColumn();

            if ($alreadyBorrowed == 0) {
                $insert = $conn->prepare('INSERT INTO borrow (borrowcode, member_id, item_id, stock_id, user_id, room_assigned, time_limit, quantity) VALUES(?,?,?,?,?,?,?,?)');
                $insert->execute(array($code, $value['member_id'], $value['item_id'], $value['stock_id'], $sessionid, $value['assign_room'], $value['time_limit'], $quantityBorrowed));
                $count = $insert->rowCount();
                $borrowIds[] = $conn->lastInsertId();

                if ($count > 0) {
                    $update = $conn->prepare('UPDATE item_stock SET items_stock = (items_stock - ?) WHERE id = ?');
                    $update->execute(array($quantityBorrowed, $value['stock_id']));
                    $row = $update->rowCount();
                }
            }
        }
