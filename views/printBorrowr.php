...
$getBorrowedLists = $conn->prepare("SELECT reservation.*, member.*, room.*, item.*, borrow.* 
                                   FROM reservation 
                                   LEFT JOIN member ON member.id = reservation.member_id 
                                   LEFT JOIN room ON room.id = reservation.room_id 
                                   LEFT JOIN item ON item.id = reservation.item_id 
                                   LEFT JOIN borrow ON borrow.id = reservation.id 
                                   WHERE borrow.id IN(?)");
...
<?php
// Fetch the latest reservation code
$selectLatestReservationCode = $conn->prepare('SELECT DISTINCT reservation_code FROM reservation ORDER BY reserve_date DESC LIMIT 1');
$selectLatestReservationCode->execute();
$latestReservationCode = $selectLatestReservationCode->fetchColumn();

// Fetch the latest 5 records with the same reservation code
$selectNewData = $conn->prepare('SELECT reservation.*, item.i_category 
                                FROM reservation 
                                INNER JOIN item ON reservation.item_id = item.id 
                                WHERE reservation.reservation_code = ? 
                                ORDER BY reservation.reserve_date DESC 
                                LIMIT 5');
$selectNewData->execute([$latestReservationCode]);
$newlyInsertedData = $selectNewData->fetchAll();

foreach ($newlyInsertedData as $reservedItem) {
    // Fetch item details from the database based on item_id
    $itemId = $reservedItem['item_id'];
   ...
   <tr style='mso-yfti-irow:5;height:13.0pt'>
        ...
        <td width="81%" colspan=7 style='width:81.3%;border-top:none;border-left:
            ...
            <p class=MsoNormal style='margin-bottom:0cm;margin-bottom:.0001pt'>
                ...
                ?>
                <span lang=EN-US style='font-size:10.0pt;mso-bidi-font-size:9.0pt;line-height:106%;
                font-family:"Times New Roman",serif;mso-bidi-font-weight:bold'><o:p>&nbsp;</o:p></span>
            </p>
        </td>
    </tr>
    <?php
...
?>
 ...
 <tr style='mso-yfti-irow:31;height:13.0pt'>
  ...
  <p class=MsoNormal style='margin-bottom:0cm;margin-bottom:.0001pt'><span
  lang=EN-US style='font-size:10.0pt;mso-bidi-font-size:9.0pt;line-height:106%;
  font-family:"Times New Roman",serif;mso-bidi-font-weight:bold text-align:center'><o:p>&nbsp;</o:p></span><?php echo $data[0]['m_fname']; ?> <?php echo $data[0]['m_lname']; ?></p>
  </td>
