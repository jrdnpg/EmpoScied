...
$('.client_reservation').submit(function (e) {
    ...
    formData.set('key', 'addclient_reservation');
    ...
    .done(function (data) {
        console.log(data);
        if (data == 1) {
            toastr.success('Successful. Check your request status if your request was accommodated');
            // Reset other form elements and reload the table if needed
            ...
            toastr.warning('Your request cannot process. You can only make one request per day.');
            // Reset other form elements and reload the table if needed
            $('input[name="reserved_date"]').val('');
            $('input[name="reserved_time"]').val('');
            $('select[name="reserve_room"]').val('');
            ...
            toastr.error('Not Enough quantity in stock.');
        }
    });
});
	...
	var form = '<form class="frm_add_student">\
					...
					<div class="form-group">\
						...
						<select name="s_gender" class="form-control" required>\
							<option disabled selected>Your sex</option>\
							<option>Male</option>\
