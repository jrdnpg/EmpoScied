...
function handleAjaxAndPopulateSelect(key, targetElement) {
    $.ajax({
        type: "POST",
        url: "../class/display/display",
        data: {
    ...
    .done(function(data){
        if(data && data.trim() !== ''){
            try {
                ...
                targetElement.html(select).select2({
                    placeholder: "Select item to borrow",
                    maximumSelectionLength: 5000,
                    tokenSeparators: [',', ' ']
                });
            } catch (error) {
            ...
            targetElement.html("<option>No items available</option>").select2({
                placeholder: "Select item to borrow",
                maximumSelectionLength: 5000,
                tokenSeparators: [',', ' ']
            });
...
// Usage
handleAjaxAndPopulateSelect("display_item_borrow", $('.borrowitem'));
handleAjaxAndPopulateSelect("display_equipment_all", $('.borrowitem1'));
handleAjaxAndPopulateSelect("display_equipment_new", $('.borrowitem2'));
handleAjaxAndPopulateSelect("display_item_borrow2", $('.borrowitem3'));
handleAjaxAndPopulateSelect("display_item_borrow3", $('.borrowitem4'));
